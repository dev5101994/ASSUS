...
const ActivityStats = (props) => {
    ...
    // console.log("------chartData-------", chartDate.dataset_total_activity)
    React.useEffect(() => {
        activityChartDataAPI()

    }, [])

    // API methods
    const activityChartDataAPI = async (data) => {
        let params = {
        ...
        let url = Constants.apiEndPoints.activityChart;
        // console.log("url", url);
        // console.log("params", params);
        setIsLoading(true)
        let response = await APIService.postData(url, params, UserLogin.access_token, null, "activityChartDataAPI");
        // return
    ...
    // console.log("---------", chartDate.dataset_total_activity)
                        ...
                        chartDate.labels
                            ...
                            <View style={{}}>
                                ...
                                <ScrollView
                                    horizontal={true}
                                >
                                    <RenderChartItems chartName={labels?.["activity-stats"]} item={chartDate.dataset_total_pending} dateLabel={labels?.["upcoming"] ?? "Upcoming"} dataColor={Colors.primary} />
                                    <RenderChartItems item={chartDate.dataset_total_done} dateLabel={labels?.["done"] ?? "Done"} dataColor={Colors.green} />
                                    <RenderChartItems item={chartDate.dataset_total_not_applicable} dateLabel={labels?.["not-applicable"] ?? "Not Applicable"} dataColor={Colors.aqua} />
                                    <RenderChartItems item={chartDate.dataset_total_not_done} dateLabel={labels?.["not-done"] ?? "Not Done"} dataColor={Colors.gray} />
                                </ScrollView>
                                ...
                                <ScrollView
                                    horizontal={true}
                                >
                                    <RenderChartItems chartName={labels?.["patient-stats"]} item={chartDate.dataset_total_activity} dateLabel={labels?.["dataset-total-activity"] ?? "Total Activity"} dataColor={Colors.primary} />
                                    <RenderChartItems item={chartDate.dataset_total_compulsory} dateLabel={labels?.["dataset-total-compulsory"] ?? "Compulsory"} dataColor={Colors.green} />
                                    <RenderChartItems item={chartDate.dataset_total_risk} dateLabel={labels?.["is-risk"] ?? "Risky"} dataColor={Colors.aqua} />
                                </ScrollView>

                                {/* Activity Actions Stats */}
                                <ScrollView
                                    horizontal={true}
                                >
                                    <RenderChartItems chartName={labels?.["action-stats"]} item={chartDate.dataset_total_completed_by_staff_on_time} dateLabel={labels?.["dataset-total-completed-by-staff-on-time"] ?? "Completed By Staff On Time"} dataColor={Colors.primary} />
                                    <RenderChartItems item={chartDate.dataset_total_completed_by_staff_not_on_time} dateLabel={labels?.["dataset-total-completed-by-staff-not-0n-time"] ?? "Completed By Staff Not On Time"} dataColor={Colors.green} />
                                    <RenderChartItems item={chartDate.dataset_total_completed_by_patient_itself} dateLabel={labels?.["dataset-total-completed-by-patient-itself"] ?? "Completed By Patient Itself"} dataColor={Colors.aqua} />
                                    <RenderChartItems item={chartDate.dataset_total_patient_did_not_want} dateLabel={labels?.["dataset-total-patient-did-not-want"] ?? "Patient Did Not Want"} dataColor={Colors.gray} />
                                    <RenderChartItems item={chartDate.dataset_total_not_done_by_employee} dateLabel={labels?.["dataset-total-not-done-by-employee"] ?? "Not Done By Employee"} dataColor={Colors.green} />
                                </ScrollView>
                                ...
                                {/*
                                    ...
                                    <LineChart
                                        data={{
                                            ...
                                            datasets: [
                                                {
                                                    data: [...chartDate.dataset_total_activity],
                                                    strokeWidth: 2,
                                                    color: (opacity = 1) => Colors.primary,
                                                },
                                                {
